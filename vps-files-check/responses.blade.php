        ...
        <div>
            <div class="flex items-center justify-between border-b border-gray-200">
                ...
                <div class="flex items-center gap-2">
                    @if($totalResponses > 0)
                    <a href="{{ route('forms.responses.export.summary', $form->id) }}" id="export-summary-btn" class="inline-flex items-center space-x-2 px-4 py-2 text-sm font-medium text-red-600 border border-red-600 rounded-lg hover:bg-red-50 transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                        </svg>
                        <span>Export Summary ke Word</span>
                    </a>
                    <a href="{{ route('forms.responses.export.individual', $form->id) }}" id="export-individual-btn" class="items-center space-x-2 px-4 py-2 text-sm font-medium text-red-600 border border-red-600 rounded-lg hover:bg-red-50 transition-colors hidden">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                        </svg>
                        <span>Export Individual ke Word</span>
                    </a>
                    @else
                    <button disabled class="inline-flex items-center space-x-2 px-4 py-2 text-sm font-medium text-gray-400 border border-gray-300 rounded-lg cursor-not-allowed opacity-50">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                        </svg>
                        <span>Export (Belum ada jawaban)</span>
                    </button>
                    @endif
                </div>
            </div>
...
<script type="application/json" id="summary-data-json">
    @json($questionSummaries)
</script>
<script>
    ...
    const summaryData = JSON.parse(document.getElementById('summary-data-json').textContent);
...
<script type="application/json" id="individual-responses-json">
    @json($individualResponses)
</script>
<script>
    const individualResponses = JSON.parse(document.getElementById('individual-responses-json').textContent);
    let currentIndex = 0;
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Initialize: Show summary button, hide individual button on page load
        if (exportSummaryBtn) {
            exportSummaryBtn.classList.remove('hidden');
            exportSummaryBtn.classList.add('inline-flex');
        }
        if (exportIndividualBtn) {
            exportIndividualBtn.classList.add('hidden');
            exportIndividualBtn.classList.remove('inline-flex');
        }
        ...
        if (summaryTab && individualTab) {
            ...
            individualTab.addEventListener('click', () => {
                ...
                // Show/hide export buttons
                if (exportSummaryBtn) {
                    exportSummaryBtn.classList.add('hidden');
                    exportSummaryBtn.classList.remove('inline-flex');
                }
                if (exportIndividualBtn) {
                    exportIndividualBtn.classList.remove('hidden');
                    exportIndividualBtn.classList.add('inline-flex');
                }
            });
        }
    });
